        ...
        *this << decision << std::endl;
        ...
        case IMAGE_INLINE_FILE:
            if ( isBinary() )
            {
				...
				if (!infile.is_open() && img->getFileName().size() > 0)
				{
					osg::ref_ptr<osgDB::Options> spOptions = new osgDB::Options();
					spOptions = dynamic_cast<osgDB::Options*> (_options->clone(osg::CopyOp::DEEP_COPY_ALL));
					spOptions->setOptionString("JPEG_QUALITY 75");

					char szTempDir[MAX_PATH];
					GetTempPath(MAX_PATH - 1, szTempDir);
					std::stringstream sstr;
					sstr << szTempDir << osgDB::getStrippedName(img->getFileName()) << ::time(nullptr) << osgDB::getFileExtensionIncludingDot(img->getFileName());
					fullPath = sstr.str();
					if (osgDB::writeImageFile(*img, fullPath, spOptions.get()))
					{
						infile.open(fullPath.c_str(), std::ios::in | std::ios::binary);
						bDeleteFile = true;
					}
					else
					{
